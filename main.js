...
const TimeInOneCell = 420;
const AroundPivotLimit = 8;
...
class GameMap {
    ...
    parseTicktack(id, res) {
        //console.log(id, res.map_info.players);
    ...
    recheckCanBomb() {
        const canBomb = this.mapInfo.bombs.filter(b => b.playerId === this.playerId).length === 0;
        if (canBomb) {
    ...
    updateMap(tag, player_id) {
        ...
        } else if (player_id !== this.playerId) {
    ...
    readMap2(withMystic = false) {
        ...
        for (let spot of attackSpots) {
            ...
            //const points = spot.boxes + spot.bonusPoints * 0.2 - spot.distance;
            //const goodSpotPoints = goodSpot.boxes + goodSpot.bonusPoints * 0.2 - goodSpot.distance;
            const points = spot.boxes;
            const goodSpotPoints = goodSpot.boxes;
            if (goodSpotPoints < points) {
                goodSpot = spot;
            }
        }
    ...
    canWalkThrough(bombTime, distance) {
        return distance * 300 + 650 < bombTime;
    }

    readThisNode(node) {
        this.countBoxHits(node);
        if (node.boxes === 0 && !node.attackThis) {
            this.emptyCells.add(node.val);
        }
    }
    ...
    goToGoodSpot() {
        ...
        if (goodSpot1) {
            //console.log('good spot 1', this.to2dPos(goodSpot1.val));
            const path = getPathFromRoot(goodSpot1);
            this.drivePlayer(path, goodSpot1);
            this.storeRoadMap([goodSpot1]);
        } else if (goodSpot2) {
            const path = getPathFromRoot(goodSpot2);
            this.drivePlayer(path, goodSpot2);
            this.storeRoadMap([goodSpot2]);
        }
    }
    ...
    findGoodSpot(position, withMystic = false, withTeleport = false) {
        ...
        while (queue.length) {
            ...
            if (!goodSpots.length || currentNode.bonusPoints || currentNode.boxes || currentNode.attackThis) {
        ...
        for (let spot of goodSpots) {
            ...
            //const points = spot.boxes * 0.7 + spot.bonusPoints * 0.2;
            //const goodSpotPoints = spot.boxes * 0.7 + goodSpot.bonusPoints * 0.2;
            const points = spot.boxes;
            const goodSpotPoints = spot.boxes;
            if (goodSpotPoints < points) {
                goodSpot = spot;
            }
        }
    ...
    findSafePlace(position, dangerSpots, initDistance = 0, withMystic = false) {
        const goodSpots = [];
        ...
        for (let spot of goodSpots) {
            ...
            if (
                (
                    ...
                    (points < 1 && goodSpotPoints < points)
                ) &&
